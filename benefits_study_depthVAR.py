...
SAVE_DATA_PLOT_N = f'data/data_plot_n_d{d}_indim{intern_dim}_depthVARIATION.npy'
SAVE_DATA_PLOT_SGD = f'data/data_plot_sgd_d{d}_indim{intern_dim}_depthVARIATION.npy'
SAVE_DATA_PLOT_RIDGE = f'data/data_plot_ridge_d{d}_indim{intern_dim}_depthVARIATION.npy'
...
if COMPUTE_DATA_PLOT:
    ### Study
    # Initialisation
    y_plot = np.zeros((len(depths), len(all_which_h), len(all_which_w), len(n_sgd))) # (h,w, n_sgd)
    sgd_risks = np.zeros((len(depths), len(all_which_h), len(all_which_w), len(n_sgd)))
...
for i, which_h in enumerate(all_which_h):
    if PLOT_ONLY_N:
        for j,which_w in enumerate(all_which_w):
            for k, depth in enumerate(depths):
                plt.plot(n_sgd, y_plot[k,i,j,:], label=f'depth: {depth}')
            ...
            plt.suptitle('SGD vs Ridge - effect of depth; H:'+H_LABELS[which_h-1]+'; '+W_LABELS[j])
            plt.savefig(SAVE_DIR_FIG+f'benefits_partial_H{which_h}_w{which_w}_d{d}_indim{intern_dim}_depthVARIATION')
            plt.show()
    else:
        ...
        plt.suptitle('SGD vs Ridge - effect of depth; H:'+H_LABELS[which_h-1])
        plt.savefig(SAVE_DIR_FIG+f'benefits_H{which_h}_d{d}_indim{intern_dim}_depthVARIATION')
        plt.show()
