...
intern_dim = 10  # fixed internal dimension for comparison
depths = np.arange(1, 11)  # depths from 1 to 10
...
if COMPUTE_DATA_PLOT:
    ### Study
    y_plot = np.zeros((len(depths), len(all_which_h), len(all_which_w), len(n_sgd)))  # (depth, h, w, n_sgd)
...
for i, which_h in enumerate(all_which_h):
    ...
    for j, which_w in enumerate(all_which_w):
        for k, depth in enumerate(depths):
            axs[j].plot(n_sgd, y_plot[k, i, j, :], label=f'depth: {depth}')
    ...
    plt.suptitle('SGD vs Ridge - effect of depth; H:' + H_LABELS[i])
