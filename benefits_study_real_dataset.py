...
plt.plot(n_sgd, y_plot)
plt.grid(color='black', which="both", linestyle='--', linewidth=0.2)
plt.xlabel(r'$N_{SGD}$')
plt.ylabel(r'$N_{Ridge}$')
plt.ylim(0, N_max_ridge)
plt.title(f'SGD vs Ridge on Insurance Regression dataset')
plt.savefig(SAVE_DIR_FIG + 'benefits_Insurance_plot1.png')
plt.show()

# Second plot
plt.figure(figsize=(8, 8))
plt.plot(n_sgd, sgd_risks, label='SGD')
plt.plot(n_ridge, ridge_risks, linestyle='--', label='Ridge')
plt.grid(color='black', which="both", linestyle='--', linewidth=0.2)
plt.legend()
plt.yscale('log')
plt.xlabel('N')
...
plt.title(f'SGD vs Ridge on Insurance Regression dataset')
plt.savefig(SAVE_DIR_FIG + 'benefits_Insurance_plot2.png')
plt.show()
